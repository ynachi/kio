...
namespace bitcask
{
    HintFile::HintFile(const int fd, const uint64_t file_id, Worker& io_worker, BufferPool& bp, BitcaskConfig& config) :
    ...
    Task<Result<void>> HintFile::async_write(const HintEntry&& entry) const
    {
        ...
        KIO_TRY(co_await io_worker_.async_write_exact(fd_, data));
        co_return {};
    }

    Task<Result<void>> HintFile::async_read(KeyDir& keydir) const
    {
        const auto buf = KIO_TRY(co_await read_file_content(io_worker_, fd_));
        if (buf.empty())
        {
            co_return {};
        }
