...
enum class Level : uint8_t { Debug = 0, Info = 1, Warn = 2, Error = 3, Fatal = 4, Disabled = 5 };
    ...
    inline void worker_loop(std::stop_token st) {
        ...
        while (!st.stop_requested()) {
            ...
            while (true) {
                ...
                if (slot.state.load(std::memory_order_acquire) != 2) break;
                ...
                if (g_colors.load(std::memory_order_relaxed)) {
                    reset = "\033[0m";
                    switch (r.level) {
                        case Level::Debug: color = "\033[36m";   level_str = "DBG"; break;
                        case Level::Info:  color = "\033[32m";   level_str = "INF"; break;
                        case Level::Warn:  color = "\033[33m";   level_str = "WRN"; break;
                        case Level::Error: color = "\033[31m";   level_str = "ERR"; break;
                        case Level::Fatal: color = "\033[1;31m"; level_str = "FTL"; break;
                        default: break;
                    }
                } else {
                    switch (r.level) {
                        ...
                        case Level::Fatal: level_str = "FTL"; break;
                        default: break;
                    }
                }

                std::println(stderr, "{}[{}] [{:%H:%M:%S}] [{}] {}:{} | {}{}",
                             color, level_str, r.ts, r.tid, basename(r.loc.file_name()),
                             r.loc.line(), std::string_view(r.msg.data(), r.len), reset);
        ...
        while (true) {
            ...
            std::println(stderr, "[{}] [{:%H:%M:%S}] [{}] {}:{} | {}",
                         level_str, r.ts, r.tid, basename(r.loc.file_name()),
...
inline void set_level(Level level) noexcept { g_runtime_level.store(level, std::memory_order_relaxed); }
inline void set_colors(bool enable) noexcept { g_colors.store(enable, std::memory_order_relaxed); }
inline void start() { detail::start(); }
inline void stop() {
...
void log_impl(std::source_location loc, std::format_string<Args...> fmt, Args&&... args) noexcept {
    ...
    while (slot.state.load(std::memory_order_acquire) != 0) std::this_thread::yield();
    slot.state.store(1, std::memory_order_relaxed);
    
    auto& r = slot.record;
    r.level = L; r.loc = loc; r.ts = std::chrono::system_clock::now(); r.tid = std::this_thread::get_id();
