...
inline auto accept(IoContext& ctx, Fd& server_fd, SocketAddress& client_addr, const int flags = 0)
{
    return IoAwaiter(
        ctx,
        URING_TRACE_OP_ARG("accept")[raw_fd = server_fd.fd, &client_addr, flags](io_uring_sqe * sqe) {
...
inline auto accept(IoContext& ctx, Fd& server_fd, const int flags = SOCK_NONBLOCK | SOCK_CLOEXEC)
{
    return IoAwaiter(
        ctx,
        URING_TRACE_OP_ARG("accept")[raw_fd = server_fd.fd, flags](io_uring_sqe * sqe) {
...
inline auto connect(IoContext& ctx, Fd& fd, const SocketAddress& addr)
{
    ...
    return IoAwaiter(
        ctx,
        URING_TRACE_OP_ARG("connect")[raw_fd = fd.fd, addr](io_uring_sqe * sqe) mutable {
...
inline auto read(IoContext& ctx, Fd& fd, std::span<std::byte> buf, off_t offset = -1)
{
    return IoAwaiter(
        ctx,
        URING_TRACE_OP_ARG("read")[raw_fd = fd.fd, buf, offset](io_uring_sqe * sqe) {
...
inline auto readv(IoContext& ctx, Fd& fd, std::span<const iovec> iovecs, off_t offset = -1)
{
    return IoAwaiter(
        ctx,
        URING_TRACE_OP_ARG("readv")[raw_fd = fd.fd, iovecs, offset](io_uring_sqe * sqe) {
...
inline auto write(IoContext& ctx, Fd& fd, std::span<const std::byte> buf, off_t offset = -1)
{
    return IoAwaiter(
        ctx,
        URING_TRACE_OP_ARG("write")[raw_fd = fd.fd, buf, offset](io_uring_sqe * sqe) {
...
inline auto writev(IoContext& ctx, Fd& fd, std::span<const iovec> iovecs, off_t offset = -1)
{
    return IoAwaiter(
        ctx,
        URING_TRACE_OP_ARG("writev")[raw_fd = fd.fd, iovecs, offset](io_uring_sqe * sqe) {
...
inline auto open(IoContext& ctx, std::filesystem::path path, const int flags, const mode_t mode = 0644)
{
    return IoAwaiter(
        ctx,
        URING_TRACE_OP_ARG("open")[path, flags, mode](io_uring_sqe * sqe) {
...
inline auto remove(IoContext& ctx, std::filesystem::path path)
{
    return IoAwaiter(
        ctx,
        URING_TRACE_OP_ARG("unlink")[path](io_uring_sqe * sqe) {
...
inline auto fsync(IoContext& ctx, Fd& fd, const bool full_sync = false)
{
    return IoAwaiter(
        ctx,
        URING_TRACE_OP_ARG("fsync")[raw_fd = fd.fd, full_sync](io_uring_sqe * sqe) {
...
inline auto fallocate(IoContext& ctx, Fd& fd, const int mode, const off_t offset, const off_t len)
{
    return IoAwaiter(
        ctx,
        URING_TRACE_OP_ARG("fallocate")[raw_fd = fd.fd, mode, offset, len](io_uring_sqe * sqe) {
...
inline auto ftruncate(IoContext& ctx, Fd& fd, const off_t len)
{
    return IoAwaiter(
        ctx,
        URING_TRACE_OP_ARG("ftruncate")[raw_fd = fd.fd, len](io_uring_sqe * sqe) {
...
inline auto poll(IoContext& ctx, Fd& fd, const unsigned poll_mask)
{
    return IoAwaiter(
        ctx,
        URING_TRACE_OP_ARG("poll")[raw_fd = fd.fd, poll_mask](io_uring_sqe * sqe) {
...
auto timeout(IoContext& ctx, const std::chrono::duration<Rep, Period> dur)
{
    return IoAwaiter(
        ctx,
        URING_TRACE_OP_ARG(
            "timeout")[ts = __kernel_timespec{.tv_sec = std::chrono::duration_cast<std::chrono::seconds>(dur).count(),
                                              .tv_nsec =
