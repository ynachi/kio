...
namespace kio
{
    ...
    public:
        ...
        {
            Append(std::span{reinterpret_cast<const std::byte*>(sv.data()), sv.size()});
        }
    ...
    struct FDGuard
    {
        ...
        /// Transfers ownership to the caller. The Guard no longer closes the FD.
    ...
    /// @note Requires files to be registered with IoContext::RegisterFiles() first.
    ///       Uses fixed file optimization for reduced kernel overhead.
    [[nodiscard]] inline WriteFixedOp AsyncWriteFixed(IoContext& ctx, int idx, std::span<const std::byte> buf,
                                                      off_t off)
    {
    ...
    inline CloseOp AsyncClose(IoContext& ctx, const FDGuard& fd)
    {
        return CloseOp(ctx, fd);
    }
