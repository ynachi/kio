...
constexpr LevelConfig kCfg[] = {
    {"DBG", "\033[36m"}, {"INF", "\033[32m"}, {"WRN", "\033[33m"},
    {"ERR", "\033[31m"}, {"FTL", "\033[1;31m"}};
...
struct SPSC {
  ...
  bool try_push(const Record &r) noexcept {
    ...
    if ((h - t) >= kQueueSize) return false;
...
inline void logger_loop(int out_fd) {
  while (true) {
    drain_all(out_fd);
      const uint32_t current_epoch = g_epoch.load(std::memory_order_acquire);
    if (!g_running.load(std::memory_order_acquire)) {
        break;
    }
    g_epoch.wait(current_epoch, std::memory_order_relaxed);
  }
  drain_all(out_fd);
}

template <typename... Args>
inline void format_into(Record &r, Level lvl, std::source_location loc,
  ...
  if (colors) {
    res = std::format_to_n(p, end - p, "{}[{}] [{:%T}.{:03}] [{}] {}:{} | ",
        cfg.color, cfg.label,
        secs, millis,
...
inline void start(int out_fd = STDERR_FILENO) {
  ...
  if (bool expected = false; !detail::g_running.compare_exchange_strong(expected, true)) return;

  detail::g_thread = std::jthread([out_fd] { detail::logger_loop(out_fd); });
}

inline void stop() {
  if (!detail::g_running.exchange(false)) return;
  detail::wake_logger();
  if (detail::g_thread.joinable()) detail::g_thread.join();
}
