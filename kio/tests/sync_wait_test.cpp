...
{
    auto task = []() -> Task<std::string> { co_return "Hello, World!"; };
...
TEST(SyncWaitTest, ExceptionPropagation)
{
    ...
    EXPECT_THROW({ SyncWait(task()); }, std::runtime_error);
}
...
TEST(SyncWaitTest, VoidTaskExceptionPropagation)
{
    ...
    EXPECT_THROW({ SyncWait(task()); }, std::logic_error);
}

// Test: Nested tasks
TEST(SyncWaitTest, NestedTasks)
{
    auto inner_task = []() -> Task<int> { co_return 10; };
...
{
    auto task_factory = [](const int value) -> Task<int> { co_return value * 2; };
...
{
    auto task = []() -> Task<int> { co_return 123; };
...
TEST(SyncWaitTest, StructReturnValue)
{
    ...
    auto task = []() -> Task<TestData> { co_return TestData{42, "test"}; };
...
TEST(SyncWaitTest, LargeObjectReturn)
{
    struct LargeObject
    {
        ...
        LargeObject() { std::ranges::fill(data, 42); }
    };

    auto task = []() -> Task<LargeObject> { co_return LargeObject{}; };
...
TEST(SyncWaitTest, MultipleExceptions)
{
    ...
    EXPECT_THROW({ SyncWait(task1()); }, std::runtime_error);

    EXPECT_THROW({ SyncWait(task2()); }, std::logic_error);
}
...
TEST(SyncWaitTest, NestedTaskWithException)
{
    ...
    EXPECT_THROW({ SyncWait(outer_task()); }, std::runtime_error);
}
...
TEST(SyncWaitTest, EmptyVoidTask)
{
    ...
    EXPECT_NO_THROW({ SyncWait(task()); });
}

// Test: Task returning bool
TEST(SyncWaitTest, BoolReturnValue)
{
    auto task_true = []() -> Task<bool> { co_return true; };

    auto task_false = []() -> Task<bool> { co_return false; };
...
{
    auto task = []() -> Task<int> { co_return 0; };
...
{
    auto helper = [](int value) -> Task<int> { co_return value; };
...
{
    auto task_factory = [](const int i) -> Task<int> { co_return i* i; };
