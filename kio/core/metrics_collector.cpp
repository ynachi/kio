...
void WorkerMetricsCollector::Collect(MetricSnapshot& snapshot)
{
    ...
    const WorkerStats kStats = SyncWait(get_stats_task(worker_));
    ...
    auto& bytes_read_family =
        snapshot.BuildFamily("kio_worker_bytes_read_total", "Total bytes read by kio workers", MetricType::kCounter);
    bytes_read_family.Add(
        {
            {"worker_id", id_str}
    },
        static_cast<double>(kStats.bytes_read_total));
    ...
    bytes_written_family.Add(
        {
            {"worker_id", id_str}
    },
        static_cast<double>(kStats.bytes_written_total));

    // Operation count metrics
    auto& read_ops_family =
        snapshot.BuildFamily("kio_worker_read_ops_total", "Total read operations by kio workers", MetricType::kCounter);
    read_ops_family.Add(
        {
            {"worker_id", id_str}
    },
        static_cast<double>(kStats.read_ops_total));

    auto& write_ops_family = snapshot.BuildFamily("kio_worker_write_ops_total", "Total write operations by kio workers",
                                                  MetricType::kCounter);
    write_ops_family.Add(
        {
            {"worker_id", id_str}
    },
        static_cast<double>(kStats.write_ops_total));
    ...
    connections_accepted_family.Add(
        {
            {"worker_id", id_str}
    },
        static_cast<double>(kStats.connections_accepted_total));
    ...
    connect_ops_family.Add(
        {
            {"worker_id", id_str}
    },
        static_cast<double>(kStats.connect_ops_total));

    auto& open_ops_family =
        snapshot.BuildFamily("kio_worker_open_ops_total", "Total open operations by kio workers", MetricType::kCounter);
    open_ops_family.Add(
        {
            {"worker_id", id_str}
    },
        static_cast<double>(kStats.open_ops_total));

    // Coroutine metrics
    auto& coroutine_resizes_family = snapshot.BuildFamily(
        "kio_worker_coroutine_pool_resizes_total", "Total coroutine pool resizes by kio workers", MetricType::kCounter);
    coroutine_resizes_family.Add(
        {
            {"worker_id", id_str}
    },
        static_cast<double>(kStats.coroutines_pool_resize_total));
    ...
    active_coroutines_family.Add(
        {
            {"worker_id", id_str}
    },
        static_cast<double>(kStats.active_coroutines));
    ...
    errors_family.Add(
        ...
        static_cast<double>(kStats.io_errors_total));
}
