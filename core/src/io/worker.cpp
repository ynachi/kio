...
namespace kio::io {
    ...
    uint64_t Worker::get_op_id() {
        ...
        if (op_data_pool_.size() >= config_.max_op_slots) {
            ALOG_ERROR("Op slot pool exhausted ({} ops). Consider increasing default_op_slots.",
                       op_data_pool_.size());
            throw std::runtime_error("Op slot pool exhausted");
        }

        const auto cur_size = op_data_pool_.size();
        size_t new_size = std::min(static_cast<size_t>(static_cast<float>(cur_size) * config_.op_slots_growth_factor),
                                   config_.max_op_slots);
    ...
    Worker::Worker(const size_t id, const WorkerConfig &config,
                   std::function<void(Worker &)> worker_init_callback) : config_(config), id_(id),
                                                                         task_queue_(config.task_queue_capacity),
                                                                         worker_init_callback_(
                                                                             std::move(worker_init_callback)) {
        config_.check();
