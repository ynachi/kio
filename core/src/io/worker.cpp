...
namespace kio::io
{
    ...
    void Worker::loop_forever()
    {
        ...
        try
        {
            ...
            {
                // TODO: Now use a blocking call because of the eventfd
                // if (auto sqe_num = io_uring_submit_and_wait(&ring_, 1); sqe_num > 0)
    ...
    void Worker::process_completions()
    {
        ...
        io_uring_for_each_cqe(&ring_, head, cqe)
        {
            ...
            {
                spdlog::trace("Worker {} waked up", id_);
    ...
    Worker::~Worker()
    {
        ...
        spdlog::debug("Worker {} destructor finished", id_);
    }
    ...
    void Worker::wakeup_write()
    {
        ...
        spdlog::trace("Worker {} wrote to wakeup fd", id_);
    }
    ...
    void Worker::post(std::coroutine_handle<> h)
    {
        ...
        {
            spdlog::error("Worker {} task queue is full. Dropping task.", id_);
        ...
        {
            spdlog::warn("Worker {}: Failed to write to eventfd: {}", id_, strerror(errno));
        }
    }
