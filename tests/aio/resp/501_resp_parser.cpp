...
protected:
    ...
    void Feed(std::string_view data)
    {
        ...
        parser_.Buffer().Commit();  // Make the staged data readable
    }
...
TEST_F(ParserTest, ParseSimpleString)
{
    ...
    EXPECT_EQ(parser_.Buffer().ReadableBytes(), 0);
}
...
TEST_F(ParserTest, RejectsBadBulkTerminator)
{
    ...
    EXPECT_EQ(res.error(), ParseError::InvalidProtocol);
}
...
TEST(RespParserTest, AggregateSizeOverflow)
{
    ...
    parser.Buffer().Commit();

    auto res = parser.NextFrame();
    ASSERT_FALSE(res.has_value());
    EXPECT_EQ(res.error(), ParseError::Overflow);
}
...
TEST_F(ParserTest, MaxRecursiveDepthExceeded)
{
    ...
    auto result = parser_.NextFrame();
    ASSERT_FALSE(result.has_value());
    EXPECT_EQ(result.error(), ParseError::Overflow);
}
...
TEST_F(ParserTest, MalformedInteger)
{
    ...
    auto result = parser_.NextFrame();
    ASSERT_FALSE(result.has_value());
    EXPECT_EQ(result.error(), ParseError::InvalidProtocol);
}
...
TEST_F(ParserTest, Fuzz)
{
    ...
    for (int iter = 0; iter < 10000; ++iter)
    {
        ...
        parser_.Buffer().Append(std::string_view(random_data.data(), random_data.size()));
        parser_.Buffer().Commit();
