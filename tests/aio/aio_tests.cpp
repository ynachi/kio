...
template <typename T>
T run_with_timeout(aio::IoContext& ctx, aio::Task<T>& t, std::chrono::milliseconds timeout) {
    t.start();
...
aio::Task<FileRwResult> file_rw_task(aio::IoContext& ctx, int fd) {
    ...
    auto written = co_await aio::AsyncWrite(ctx, fd, std::span<const std::byte>(data), 0);

    std::array<std::byte, 5> out{};
    auto read = co_await aio::AsyncRead(ctx, fd, std::span<std::byte>(out), 0);
...
aio::Task<SendRecvResult> send_recv_task(aio::IoContext& ctx, int fd_send, int fd_recv) {
    const std::string msg = "ping";
    auto sent = co_await aio::AsyncSend(ctx, fd_send, std::string_view{msg});

    char buf[16] = {};
    auto received = co_await aio::AsyncRecv(ctx, fd_recv, buf).WithTimeout(100ms);
...
aio::Task<aio::Result<uint64_t>> event_timeout_task(aio::IoContext& ctx, aio::Event& evt) {
    co_return co_await evt.Wait().WithTimeout(10ms);
}

aio::Task<int> offload_task(aio::IoContext& ctx, aio::BlockingPool& pool) {
