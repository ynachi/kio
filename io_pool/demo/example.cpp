...
static void storage_example()
{
    ...
    // Create a test file using raw syscall (uses errno)
...
static Task<> timeout_example_task(Executor& ex)
{
    ...
    // Timeouts return void on success, or error code on failure
    auto res = co_await timeout_ms(ex, 500);
    if (!res)
    {
        Log::error("Timeout failed: {}", res.error().message());
    }
