...
kio::Task<kio::Result<int>> FDCache::GetOrOpen(kio::IoContext& ctx, uint64_t file_id, const std::filesystem::path& path)
{
    // Cache hit
...
kio::Task<> FDCache::Remove(kio::IoContext& ctx, uint64_t file_id)
{
    if (const auto it = cache_.find(file_id); it != cache_.end())
    {
                                                      ...
                                                      const uint8_t flag)
{
    ...
    if (std::cmp_not_equal(bytes_written, entry_size))
    {
        ...
        ALOG_ERROR("Partial write: expected {} bytes, wrote {} at offset {}", entry_size, bytes_written, entry_offset);
