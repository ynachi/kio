                                                             ...
                                                             CompactionContext& c_ctx, const bool is_eof)
{
    ...
    while (in_buf.ReadableBytes() >= kEntryFixedHeaderSize)
    {
        ...
        if (out_buf.ReadableBytes() >= kDefaultOutBufWriteSize)
        {
            ...
            KIO_CO_TRY(co_await kio::AsyncWriteExact(ctx, dst_file.Fd(), out_span, c_ctx.dst_write_offset));
                                                                 ...
                                                                 DataFile& dst_file)
{
    ...
    for (const uint64_t src_id : src_file_ids)
    {
        ...
        while (!file_done)
        {
            ...
            auto bytes_read_res = co_await kio::AsyncRead(ctx, src_fd, wr_span, src_read_ptr);
            if (!bytes_read_res)
...
kio::Task<kio::Result<void>> Compactor::Compact(kio::IoContext& ctx)
{
    ...
    {
        const int dst_fd =
            KIO_CO_TRY(co_await kio::AsyncOpen(ctx, dst_path, config_.write_flags, config_.file_mode)).Get();
        ...
        ALOG_INFO("Partition {}: reclaimed {} bytes from {} files", partition_id, reclaimed, fragmented_files.size());
        co_return {};
    };
