 ...
 */
Lazy<> handleClient(next::v1::IoUringExecutor* executor, int client_fd, const std::string client_addr)
{
 ...
 */
Lazy<void> handleClientHttp(next::v1::IoUringExecutor* executor, int client_fd)
{
 ...
 */
Lazy<> acceptLoop(next::v1::IoUringExecutor* executor, int listen_fd)
{
    size_t next_ctx = 0;
    ...
    while (true)
    {
        ...
        // Donâ€™t call start() on a Lazy from outside the executor without via(); the coroutine will initially run on
        // your current thread and only switch to a worker after an await, leading to underâ€‘utilisation.
...
int main()
{
    try
    {
        ...
        // Create executor
        next::v1::IoUringExecutorConfig config;
        ...
        next::v1::IoUringExecutor executor(config);
