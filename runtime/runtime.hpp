...
private:
    void start(bool pin) {
        thread_ = std::thread([this, pin]() {
            run_thread(pin, index_);
        });
    }
...
public:
    ...
    void start(const bool pin_threads = false) {
        if (started_) return;
        started_ = true;
        for (const auto& ctx : threads_) ctx->start(pin_threads);
    ...
    void stop() {
        if (!started_) return;
        for (const auto& ctx : threads_) ctx->request_stop();
        for (const auto& ctx : threads_) ctx->join();
        started_ = false;
    }

    bool running() const {
        return started_ && std::any_of(threads_.begin(), threads_.end(),
            [](const auto& ctx) { return ctx->running(); });
    }
